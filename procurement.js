(function () {
        ...
        <div class="grid-2col grid-2col-uneven">
          <div>
            ...
            <div class="panel">
              ...
              ${renderOpenPOTable(model.openPurchaseOrders, writeEnabled)}
            </div>
          </div>
  ...
  function renderOpenPOTable(pos, writeEnabled) {
    if (!pos || pos.length === 0) {
      return `<div class="empty-state">${
        writeEnabled
          ? "No open purchase orders. Generate one from a purchase request above, or add one directly in the Data Console."
          : "No open purchase orders. Add a row to the Purchase Orders tab in your Google Sheet if you'd like one to appear here."
      }</div>`;
    }
    const rows = pos
      .map(
