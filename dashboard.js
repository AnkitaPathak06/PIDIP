    ...
    <div class="page">
      ...
      <div class="grid-2col grid-2col-uneven">
        ...
        <div class="panel">
          ...
          <div class="supplier-insights-mini">
            <h4>Supplier Insights</h4>
            ${renderSupplierInsights(model.supplierInsights)}
          </div>
        </div>
      </div>

      <div class="quick-kpis quick-kpis-5">
        ...
        <div><strong>${summary.openPurchaseOrders}</strong><span>Open Purchase Orders</span></div>
        <div><strong>${summary.inventoryTurnover}</strong><span>Inventory Turnover</span></div>
      </div>
    </div>
  `;
...
function renderSupplierInsights(insights) {
  if (!insights) return `<div class="empty-state">No supplier data available.</div>`;
  return `
    <ul class="insights-list">
      <li><span>Highest Reliability</span><strong>${insights.highestReliability ? Fmt.escapeHtml(insights.highestReliability.name) + " — " + insights.highestReliability.value + " score" : "—"}</strong></li>
      <li><span>Fastest Lead Time</span><strong>${insights.fastestLeadTime ? Fmt.escapeHtml(insights.fastestLeadTime.name) + " — " + insights.fastestLeadTime.value + " days" : "—"}</strong></li>
      <li><span>Lowest MOQ</span><strong>${insights.lowestMoq ? Fmt.escapeHtml(insights.lowestMoq.name) + " — MOQ " + insights.lowestMoq.value : "—"}</strong></li>
      <li><span>Requiring Attention</span><strong>${insights.requiringAttention}</strong></li>
      <li><span>Emergency Coverage</span><strong>${insights.emergencyCoverage}</strong></li>
    </ul>`;
}
