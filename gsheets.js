...
const GSheets = (() => {
  ...
  async function fetchSheet(sheetName) {
    ...
    const rows = (json.table.rows || []).map((r, idx) => {
      const obj = { _rowKey: `${sheetName}#${idx}` };
    ...
    // drop fully blank rows (ignore the internal _rowKey when checking)
    return rows.filter((row) =>
      Object.entries(row).some(([k, v]) => k !== "_rowKey" && v !== null && v !== "" && v !== undefined)
    );
  }
