...
const App = (() => {
  ...
  const ROUTES = {
    ...
    dataconsole: { title: "Data Console", render: Pages.dataconsole },
  ...
  function updateRefreshStamp() {
    ...
    if (badge) {
      badge.textContent = "Editable · Changes stay on this device";
    }

    const clearBtn = document.getElementById("clear-changes-btn");
    if (clearBtn) {
      clearBtn.style.display = SessionStore.hasAnyChanges() ? "inline-block" : "none";
    }
  }
  ...
  async function init() {
    ...
    try {
      ...
      // Layer this visitor's own local edits on top of the live sheet data.
      // These edits live only in this browser and are never sent anywhere.
      Object.keys(CONFIG.SHEETS).forEach((key) => {
        const sheetName = CONFIG.SHEETS[key];
        raw[key] = SessionStore.applyOverlay(sheetName, raw[key] || []);
      });
      state.raw = raw;
  ...
  function start() {
    ...
    document.getElementById("clear-changes-btn").addEventListener("click", () => {
      if (!confirm("Clear all your local changes and go back to the live sheet data? This can't be undone.")) return;
      SessionStore.resetAll();
      Fmt.toast("Your local changes have been cleared.", "success");
      init();
    });
    init();
