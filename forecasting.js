  ...
  function renderDetail(materialId) {
      ...
      <div class="panel">
        ...
        <div class="table-wrap">
          <table id="history-table">
            <thead><tr><th>Date</th><th>Quantity Consumed</th></tr></thead>
            <tbody></tbody>
          </table>
        </div>
        ...
        ${
          WriteAPI.isConfigured()
            ...
            : `<div class="write-disabled-note">This is a read-only view. To log new consumption, add a row to the Consumption History tab in your Google Sheet directly.</div>`
        }
      </div>
    `;
      ...
      document.getElementById("log-consumption-btn").addEventListener("click", async () => {
        ...
        try {
          await WriteAPI.appendRow(CONFIG.SHEETS.consumption, {
            Date: Fmt.localDateStr(new Date(y, mo - 1, d)),
    ...
    function renderHistoryTable() {
      ...
      if (historyState.search) {
        ...
        rows = rows.filter((h) => Fmt.date(h.date).toLowerCase().includes(q));
      }
      ...
      document.querySelector("#history-table tbody").innerHTML =
        pageRows.length === 0
          ? `<tr><td colspan="2" class="empty-state">No consumption records match your search.</td></tr>`
          : pageRows
              .map((h) => `<tr><td>${Fmt.date(h.date)}</td><td>${Fmt.num(h.qty, 1)}</td></tr>`)
              .join("");
