  ...
  Crud.renderTable = function (container, tableConfig, rows) {
    ...
    function filtered() {
      ...
      return rows.filter((r) =>
        Object.entries(r).some(([k, v]) => k !== "_rowKey" && String(v ?? "").toLowerCase().includes(q))
      );
    }
    ...
    function render() {
      ...
      container.innerHTML = `
        <div class="table-toolbar crud-toolbar">
          ...
          ${writeEnabled ? `<button class="btn btn-primary" id="crud-add">+ Add Record</button>` : ""}
        </div>
        <div class="table-wrap">
          <table>
            <thead><tr>${tableConfig.columns.map((c) => `<th>${Fmt.escapeHtml(c.label)}</th>`).join("")}${writeEnabled ? "<th></th>" : ""}</tr></thead>
            <tbody>
              ${
                pageRows.length === 0
                  ? `<tr><td colspan="${tableConfig.columns.length + (writeEnabled ? 1 : 0)}" class="empty-state">No records${state.search ? " match your search" : ""}.</td></tr>`
                  : pageRows
                      .map(
                        (row) => `
                <tr>
