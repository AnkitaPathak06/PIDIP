...
const Engine = (() => {
  ...
  function forecastForMaterial(consumptionRows, rules, today) {
    const normalized = consumptionRows.map((r) => ({ ...r, Date: Fmt.toDate(r.Date) })).filter((r) => r.Date);
  ...
  function buildModel(raw, rules) {
    ...
    const materials = Array.from(materialsById.values()).map((mat) => {
      ...
      const expiryDate = Fmt.toDate(inv["Expiry Date"]);
      ...
      return {
        ...
        mfgDate: Fmt.toDate(inv["Manufacturing Date"]),
        expiryDate,
        daysToExpiry,
        isNearExpiry,
        isCriticalExpiry,
    ...
    // Purchase Orders (optional 7th tab — if it doesn't exist yet in the
    // sheet, gsheets.js already returns an empty array for it gracefully;
    // adding a PO via the app stores it locally in this browser only)
    const purchaseOrders = (raw.purchaseOrders || []).map((po) => ({
      ...
      expectedDate: Fmt.toDate(po["Expected Date"]),
      status: po["Status"] || "Pending",
      createdDate: Fmt.toDate(po["Created Date"]),
    }));
